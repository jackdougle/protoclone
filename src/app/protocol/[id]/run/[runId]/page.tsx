...
export default function RunPage() {
  ...
  function updateStepState(stepId: string, patch: Partial<StepState>) {
    setRun((prev) => {
      ...
      const newStates = {
        ...prev.stepStates,
        [stepId]: { ...getStepState(stepId), ...patch },
      };
  ...
  function updateNotes(stepId: string, notes: string) {
    updateStepState(stepId, { notes });
  }

  function modifySegment(stepId: string, segId: string, mod: Partial<Segment>) {
    const current = getStepState(stepId);
    updateStepState(stepId, {
      modifications: { ...current.modifications, [segId]: mod },
    });
  }

  async function handleComplete() {
    if (!run) return;
    await api.updateRun(protocolId, runId, {
      status: 'completed',
    ...
    <div className="min-h-screen">
      <header className="border-b border-neutral-200 sticky top-0 bg-white z-10">
        <div className="max-w-4xl mx-auto px-6 py-3 flex items-center gap-4">
          ...
          <span className="text-xs text-neutral-400 ml-2">Running protocol</span>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <p className="text-xs text-neutral-500 mb-6">Started {new Date(run.startedAt).toLocaleString()}</p>
                ...
                <div className="flex items-start gap-3 px-4 py-3">
                  ...
                  >
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
                      state.completed ? 'border-green-500 bg-green-500' : 'border-neutral-300 hover:border-neutral-400'
                    }`}>
                      {state.completed && (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`flex flex-wrap items-center gap-0.5 leading-relaxed text-sm ${state.completed ? 'line-through text-neutral-400' : ''}`}>
                      {step.segments.map((seg) => {
                        ...
                        if (modified.type === 'text') {
                          return <span key={seg.id}>{modified.value}</span>;
                        }
                        return (
                          <SegmentRenderer
                            key={seg.id}
                            segment={modified}
                            onClickChip={run.status !== 'completed' ? () => {
                              // Simple inline edit: prompt for new value
                    ...
                    {run.status === 'completed' && state.notes && (
                      <p className="mt-1 text-xs text-neutral-500 italic">{state.notes}</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        {run.status !== 'completed' && (
          <button
            ...
            className="mt-6 w-full py-3 bg-black text-white text-sm font-medium rounded-lg hover:bg-neutral-800 cursor-pointer"
          >
            Complete Run
          </button>
        )}
